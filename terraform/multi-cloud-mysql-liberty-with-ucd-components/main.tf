...
resource "aws_instance" "web-server" {
  ...
  provisioner "ucd" {
    agent_name      = "${var.web-server_agent_name}-${random_pet.env_id.id}"
    ucd_server_url  = "${var.ucd_server_url}"
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      ...
      curl -k -u ${var.ucd_user}:${var.ucd_password} ${var.ucd_server_url}/cli/agentCLI?agent=${var.web-server_agent_name}.${self.private_ip}  -X DELETE
...
resource "ibm_compute_vm_instance" "db-server" {
  ...
  provisioner "ucd" {
    agent_name      = "${var.db-server_agent_name}-${random_pet.env_id.id}"
    ucd_server_url  = "${var.ucd_server_url}"
    ucd_user        = "${var.ucd_user}"
    ucd_password    = "${var.ucd_password}"
  }
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      echo Going to delete an agent!
      curl -k -u ${var.ucd_user}:${var.ucd_password} ${var.ucd_server_url}/cli/agentCLI?agent=${var.db-server_agent_name}.${self.ipv4_address_private}  -X DELETE
      echo deleted an agent!
EOT
  }
}
...
resource "ucd_agent_mapping" "db-server_agent" {
  depends_on = [ "ibm_compute_vm_instance.db-server" ]
