...
resource "aws_instance" "aws_instance" {
  ...
  provisioner "ucd" {
    agent_name      = "${var.aws_instance_agent_name}.${random_id.aws_instance_agent_id.dec}"
    ucd_server_url  = "${var.ucd_server_url}"
  ...
  provisioner "local-exec" {
    when = "destroy"
    command = <<EOT
    curl -k -u ${var.ucd_user}:${var.ucd_password} ${var.ucd_server_url}/cli/agentCLI?agent=${var.aws_instance_agent_name}.${random_id.aws_instance_agent_id.dec} -X DELETE
EOT
}
  tags {
...
resource "random_id" "aws_instance_agent_id" {
  byte_length = 8
}
